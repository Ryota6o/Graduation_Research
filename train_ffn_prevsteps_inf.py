...
def save_loss_plot(train_losses, valid_losses, save_path):
    ...
    # 履歴が長い場合に見やすくするため、直近のみ表示などの工夫も可能ですが
    # ここでは全履歴を表示します
...
def find_latest_checkpoint(output_dir):
    """ディレクトリ内の model_epoch_X.pth から最新のものを探す"""
...
def validate(model, loader, diffusion, device, timesteps):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            pred_x0 = model(x_t, t_input, label_y)
            loss = loss_fn(pred_x0.squeeze(-1), x_0_norm)
            acc = calculate_accuracy(pred_x0, x_0)
...
# ==========================================
def parse_args():
    ap = argparse.ArgumentParser(description="QEC Training Infinite Loop")
    ap.add_argument("--config", default="config.yaml", help="path to config.yaml")
    ap.add_argument("--results_dir", type=str, default="./result_144_12_12", help="Directory to save results") 
...
def main():
    ...
    if args.pretrained_path:
        ...
        # 指定ロードの場合はstart_epochは0にするか、必要なら手動調整
    ...
    # --- Scheduler (修正済み) ---
    # 学習率は、テストデータのロスが２０回さがらなくなったら減少させるという方式にした
    ...
    print(f"--- Starting Infinite Training Loop (Continuous Time + Self-Conditioning) ---")
    ...
    try:
        for epoch in range(start_epoch, max_epochs):
            ...
            for batch_idx, (label_y, x_0) in enumerate(pbar):
                ...
                # =========================================================
                # ★ Self-Conditioning Training Logic
                # =========================================================
                # 50% の確率で prev_x0 をゼロにする (null conditioning)
                # 50% の確率で prev_x0 を「現在のモデルの予測値」にする (self conditioning)
                
                # デフォルトはゼロ (学習初期や null条件用)
                prev_x0 = torch.zeros_like(x_0_expanded)

                # 学習の進行に合わせて徐々に予測値を加えるかの判定をする。50エポックで予測値をくわえる
                target_rate = 0.5
                warmup_epochs = 50
            ...
            if avg_valid_loss < best_val_loss:
                # print(f"★ Best updated ({best_val_loss:.6f} -> {avg_valid_loss:.6f})") 
            ...
            if current_epoch_display % 500 == 0:
                ...
                np.savez(loss_log_path, 
                         train_loss=train_loss_history, 
                         valid_loss=valid_loss_history)
                print(f"Saved loss history logs: {loss_log_path}")
