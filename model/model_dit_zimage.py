...
class DiTBlock(nn.Module):
    def __init__(self, hidden_size, num_heads, mode="cross_attn", mlp_ratio=4.0, dropout=0.1, enable_modulation=True):
        super().__init__()
        self.mode = mode
        self.enable_modulation = enable_modulation
        
        # 変調なしの場合は学習可能なパラメータを持つ標準的なLayerNormにする　
        # 変調のときはLayerNorm自信の学習パラメータをオフにする elementwise_affine:Falseは、y = xγ+βのような変換しないで、ただ正規化する
        self.norm1 = nn.LayerNorm(hidden_size, elementwise_affine=not enable_modulation, eps=1e-6)
        
        # Self-Attention
        self.attn = nn.MultiheadAttention(hidden_size, num_heads, dropout=dropout, batch_first=True)
        
        # Cross-Attention (Refinerでは通常使わないが、Main Blockとの互換性のため残す)
        ...
        # adaLN Modulation (変調ありの場合のみ定義)
        if self.enable_modulation:
            if self.mode == "cross_attn":
                # [shift, scale, gate] x [self_attn, cross_attn, mlp] -> 9 parameters
                self.adaLN_modulation = nn.Sequential(
                    nn.SiLU(),
                    nn.Linear(hidden_size, 9 * hidden_size, bias=True) 
                )
            else:
                # [shift, scale, gate] x [self_attn, mlp] -> 6 parameters
                self.adaLN_modulation = nn.Sequential(
                    nn.SiLU(),
    ...
    def forward(self, x, c=None, syndrome_emb=None):
        ...
        # --- 2. Cross-Attention (mode="cross_attn" の場合のみ) ---
        if self.mode == "cross_attn":
...
class DiffusionTransformer(nn.Module):
    def __init__(
        ...
        depth=6,
        num_heads=4,
        mode="in_context", # Z-Image的には in_context (concat) が基本
        dropout=0.1,
        n_refiner_layers=2 # Refiner層の数
    ):
        super().__init__()
        self.mode = mode
        ...
        # Embedding Layers
        self.x_embedder = nn.Linear(1, hidden_size)
        self.syndrome_embedder = nn.Linear(1, hidden_size)
        self.t_embedder = TimestepEmbedder(hidden_size)
        
        # Positional Embeddings (x と y で個別に用意)
        ...
        # --- Refiner Blocks (New!) ---
        # 1. Noise Refiner (x用): 時間tによる変調あり
        self.noise_refiner = nn.ModuleList([
            DiTBlock(
                hidden_size, num_heads, mode=None, # Refiner内はSelf-AttnのみでOK
                dropout=dropout, enable_modulation=True
            ) for _ in range(n_refiner_layers)
        ])

        # 2. Context Refiner (y用): 時間tによる変調なし (Static)
    ...
    def initialize_weights(self):
        ...
        # Main blocks and Noise Refiner (modulated)
    ...
    def forward(self, x, t, label_y):
        ...
        # 7. 出力層
        if self.mode == "in_context":
            # 出力トークンのみを取り出す (後半の x 部分)
            # 長さは num_syndrome 分だけスキップ
