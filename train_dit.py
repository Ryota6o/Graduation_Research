'''無限学習(20000epoch) + Self-Conditioning + Continuous Time'''
...
# ★ 修正: Validate関数も連続時間サンプリングに対応
def validate(model, loader, diffusion, device, criterion, timesteps, use_amp=False):
    model.eval()
    total_loss = 0.0
    total_correct = 0
    total_samples_acc = 0
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            # Continuous Time Sampling for Validation
            eps = 1e-5
            t = torch.rand((batch_size,), device=device) * (1 - 2*eps) + eps
            t_input = t * timesteps
            
            noise = torch.randn_like(x_0_expanded)
            x_t_val, _ = diffusion.q_sample_continuous(x_0_expanded, t, noise)
            
            with torch.amp.autocast('cuda', enabled=use_amp):
                # prev_x0=None (default) for standard validation
                pred_x0_val = model(x_t_val, t_input, label_y, prev_x0=None)
...
# ==========================================
def parse_args():
    ap = argparse.ArgumentParser(description="QEC Infinite Training (DiT + Self-Cond)")
    ap.add_argument("--config", default="config.yaml", help="path to config.yaml")
    ap.add_argument("--results_dir", type=str, default="./result_dit_selfcond" , help="Directory to save results") 
...
def main():
    ...
    # Continuous Time Sampling を使うので、diffusion オブジェクトもそのための準備
    ...
    # Self-Conditioningによるグラフ変更があるため、compileは注意深く行う
    try:
    ...
    try:
        for epoch in range(start_epoch, max_epochs):
            ...
            for batch_idx, (label_y, x_0) in enumerate(pbar):
                ...
                print(x_0_norm.shape, label_y.shape)
                #print(x_0_norm)
                
                # Continuous Time Sampling (0.0 ~ 1.0)
                eps = 1e-5
                t = torch.rand((x_0.shape[0],), device=device) * (1 - 2*eps) + eps
                
                # timesteps (200) をかけて入力用にする
                t_input = t * timesteps
                t_input = t_input.long().to(device) # Embedderによってはlongが必要な場合もあるが、連続時間なので本来はfloatでOK。
                # ただし TimestepEmbedder が float を期待するように実装したので
                t_input = (t * timesteps).float()
                ...
                prev_x0 = torch.zeros_like(x_0_expanded)
                
                # 確率的に自己条件付けを実行
                if np.random.rand() < current_rate:
                    with torch.no_grad():
                        with torch.amp.autocast('cuda', enabled=use_amp):
                            prev_x0 = model(x_t, t_input, label_y, prev_x0=prev_x0)
                        prev_x0 = prev_x0.detach()

                with torch.amp.autocast('cuda', enabled=use_amp):
                    pred_x0 = model(x_t, t_input, label_y, prev_x0=prev_x0)
            ...
            # --- Validation (timestepsを渡す) ---
            avg_valid_loss, avg_valid_acc, avg_valid_rec = validate(model, valid_loader, diffusion, device, criterion, timesteps, use_amp)
