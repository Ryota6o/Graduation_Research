    ...
    def decode_batch(self, label_y, num_observables, steps):
        ...
        with torch.no_grad(), torch.amp.autocast('cuda'):
            for i in range(steps):
                ...
                # --- 変更点: モデルはノイズ (epsilon) を予測すると仮定 ---
                # もしモデルの学習設定が epsilon-prediction なら、この出力はノイズです
                pred_noise = self.model(x_t, t_input, label_y, prev_x0=prev_x0)
                ...
                # --- ノイズ予測から x0 を推定する公式 ---
                # x0 = (x_t - sqrt(1 - alpha_now) * noise) / sqrt(alpha_now)
                sqrt_alpha_now = torch.sqrt(alpha_bar_now)
                sqrt_one_minus_alpha_now = torch.sqrt(1.0 - alpha_bar_now)
                
                pred_x0 = (x_t - sqrt_one_minus_alpha_now * pred_noise) / (sqrt_alpha_now + 1e-8)
                pred_x0 = torch.clamp(pred_x0, -1.0, 1.0) # 安定化のためクランプ
                
                # Self-conditioning 用に保存
                prev_x0 = pred_x0

                # --- DDIM Update (次の x_t を計算) ---
                # x_{t-1} = sqrt(alpha_next) * pred_x0 + sqrt(1 - alpha_next) * pred_noise
                sqrt_one_minus_alpha_next = torch.sqrt(1.0 - alpha_bar_next)
                x_t = torch.sqrt(alpha_bar_next) * pred_x0 + sqrt_one_minus_alpha_next * pred_noise
...
def parse_args():
    ...
    ap.add_argument("--output_csv", default="dit_noise.csv", help="Output filename for CSV table")
    ap.add_argument("--output_graph", default="dit_noise.png", help="Output filename for graph")
