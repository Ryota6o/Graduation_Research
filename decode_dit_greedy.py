...
class GreedyDiffusionDecoderDDIM:
    ...
    def decode_batch(self, label_y, num_observables, steps):
        ...
        first = True
        # 2. 逆拡散プロセス (論文 Eq. A17 を使用した事後平均による更新)
        with torch.no_grad(), torch.amp.autocast('cuda'):
            for i in range(steps):
                t_now = times[i]      # 現在の時刻 t
                t_next = times[i+1]   # 次の時刻 t-1 (ノイズが少ない方)
                
                t_batch = torch.full((batch_size,), t_now, device=self.device)
                if  first and not (first := False):
                    print(times,t_now,t_next)
                # t_next が 0 の場合でも alpha_bar を正しく取得するための処理
...
def parse_args():
    ...
    ap.add_argument("--output_csv", default="dit_144_greedy.csv", help="Output filename for CSV table")
    ap.add_argument("--output_graph", default="dit_144_greedy.png", help="Output filename for graph")
...
def main():
    ...
    # ---------------------------------------------------------
    step_list = [2] + list(range(10, 101, 10)) + [200]
