...
# ==========================================
# ★ 追加: デバッグプロット用関数 (FFN版から移植)
# ==========================================
def save_debug_plots(trace_data, steps_setting, save_dir="./graph_dit"):
    """
    大量のビットからランダムに間引きしてグラフを描画する
    steps_setting: 現在のDDIMステップ数（ファイル名用）
    """
    ...
    steps_axis = np.arange(steps_count)

    # 描画する線の本数
    NUM_LINES_TO_PLOT = 1000
    ...
    for i in range(num_plot):
        plt.plot(steps_axis, subset_data[:, i], color=colors[i], alpha=0.1, linewidth=1)
        
    plt.title(f"Trajectory Sample (Steps={steps_setting})\nRed: -> 1, Blue: -> 0")
    ...
    # ファイル名にステップ数を含める
    filename = f"debug_xt_steps_{steps_setting:03d}.png"
    plt.tight_layout()
...
class GreedyDiffusionDecoderDDIM:
    ...
    # capture_trace 引数
    def decode_batch(self, label_y, num_observables, steps, capture_trace=False):
...
def parse_args():
    ...
    ap.add_argument("--batch_size", type=int, default=1, help="inference batch size")
    ...
    ap.add_argument("--output_csv", default="dit_144_1000result.csv", help="Output filename for CSV table")
    ap.add_argument("--output_graph", default="dit_144_1000result.png", help="Output filename for graph")
...
def main():
    ...
    for steps in step_list:
        ...
        # ★ 各ステップ設定ごとに1回だけプロットするためのフラグ
        trace_captured_for_this_step = False

        with torch.no_grad():
            for batch_idx, batch in enumerate(tqdm(test_loader, desc=f"Dec (Steps={steps})")):
                ...
                if capture_this_batch:
                    predicted_bits, trace_data = decoder.decode_batch(
                        syndromes, num_observables, steps=steps, capture_trace=True
                    )
                    # ★ ここで即座に保存（もしくは後で保存）
                    save_debug_plots(trace_data, steps_setting=steps)
                else:
                    predicted_bits = decoder.decode_batch(
                        syndromes, num_observables, steps=steps, capture_trace=False
                    )
